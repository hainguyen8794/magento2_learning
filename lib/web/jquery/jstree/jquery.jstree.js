...
(function ($) {
	...
	var xsl = {
		'nest' : '<' + '?xml version="1.0" encoding="utf-8" ?>' +
			...
			'<xsl:templates match="/">' +
			'	<xsl:call-templates name="nodes">' +
			'		<xsl:with-param name="node" select="/root" />' +
			'	</xsl:call-templates>' +
			'</xsl:templates>' +
			'<xsl:templates name="nodes">' +
			'	<xsl:param name="node" />' +
			'	<ul>' +
			...
			'				</a>' +
			'			</xsl:for-each>' +
			'			<xsl:if test="$children or @hasChildren"><xsl:call-templates name="nodes"><xsl:with-param name="node" select="current()" /></xsl:call-templates></xsl:if>' +
			'		</li>' +
			'	</xsl:for-each>' +
			'	</ul>' +
			'</xsl:templates>' +
